...
\small\url{https://github.com/jeffreysupergoodatcoding/liveatc-pipeline}\ $\cdot$\
...
corpora---achieves 3.88\% word error rate (WER) on European ATC data but
degrades to 30.3\% on American ATC transmissions, exposing a systematic accent
and phraseology mismatch.
...
evidenced by the 2008 Spanair MD-82 accident and the 2023 Watsonville
mid-air collision.
...
combined ATCO2 and ATCOSIM corpora, reporting 3.88\% WER on European data and
releasing the first high-accuracy open models for ATC speech recognition.
...
combined ATCO2 and ATCOSIM corpora, achieving 3.88\% WER on European ATC
and releasing the first high-accuracy open models for this domain. Their work
...
approve the result. After review, 55 high-quality clips were accepted as
ground-truth training data. The pipeline is summarized in
...
The learning rate of $5 \times 10^{-6}$---approximately one order of magnitude
below typical Whisper fine-tuning recommendations---was chosen to minimise
...
and per-epoch early-stopping on validation loss serve as additional
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Aircraft performance}: Type-specific speed envelopes,
        ...
        categories (A--D under ICAO classification).
  \item \textbf{Communication}: Tower, ground, approach, departure, and CTAF
...
The author thanks the operators of LiveATC.net for providing public access to
